...
tar -xzf "$DOCKER_ROOTLESS_DIR/docker-rootless.tgz" -C "$BIN_DIR" --strip-components=1 docker-rootless-extras/dockerd-rootless.sh docker-rootless-extras/rootlesskit --overwrite || {
...
for bin in docker dockerd dockerd-rootless.sh rootlesskit slirp4netns; do
...
chmod +x "$BIN_DIR/docker" "$BIN_DIR/dockerd" "$BIN_DIR/dockerd-rootless.sh" "$BIN_DIR/rootlesskit" "$BIN_DIR/slirp4netns"
